...
if [ "${OS_MIN}" -lt 12 ]; then
	echo "You must be running Mac OS 10.12.x or higher to install DAio."
	echo "Exiting now."
	exit 1
fi

if [ "${DISK_AVAIL}" -lt "$DISK_MIN" ]; then
	echo "You must have at least ${DISK_MIN}GB of available storage to install DAio."
	echo "Exiting now."
	exit 1
fi
...
then
	printf "Homebrew must be installed to compile DA.IO!\\n"
...
while read -r name tester testee brewname uri; do
	...
	(( COUNT++ ))
done < "${REPO_ROOT}/scripts/DAio_build_darwin_deps"
IFS="${var_ifs}"
...
if [ $COUNT -gt 1 ]; then
	printf "\\nThe following dependencies are required to install DAio:\\n"
	printf "${DISPLAY}\\n\\n"
	...
	case $ANSWER in
		1 | [Yy]* )
			...
			brew tap DAio/DAio # Required to install mongo-cxx-driver with static library
			printf "\\nInstalling Dependencies...\\n"
			# Ignore cmake so we don't install a newer version.
			# Build from source to use local cmake; see homebrew-DAio repo for examples
