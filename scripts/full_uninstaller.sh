...
if [ -d "/usr/local/include/DAio" ] || [ -d "$HOME/opt/DAio" ] || [ $FORCED == 1 ]; then # use force for running the script directly
   printf "\nDAio installation (AND DEPENDENCIES) already found...\n"
   if [ $1 == 0 ]; then
   ...
   case $ANSWER in
      1 | [Yy]* )
         if [ -d "$HOME/opt/DAio" ] || [[ $1 == "force-new" ]]; then
            if [ $( uname ) == "Darwin" ]; then
               # gettext and other brew packages are not modified as they can be dependencies for things other than DAio
            ...
            rm -rf $HOME/opt/DAio
            rm -f $HOME/bin/DAio-launcher
            rm -rf $HOME/lib/cmake/DAios
            rm -rf $HOME/opt/llvm
            rm -f $HOME/opt/boost
         ...
         if [ -d "/usr/local/include/DAio" ] || [[ $1 == "force-old" ]]; then
            if [ "$(id -u)" -ne 0 ]; then
            ...
            rm -rf libbson-1.0 libmongoc-1.0 mongocxx bsoncxx appbase chainbase DAio.system DAiolib fc libc++ musl secp256k* 2>/dev/null
            rm -rf DAio 2>/dev/null
            popd &> /dev/null
            pushd bin &> /dev/null
            rm cleos DAio-abigen DAio-applesedemo DAio-launcher DAio-s2wasm DAio-wast2wasm DAiocpp keosd nodeos 2>/dev/null
            popd &> /dev/null
            libraries=(
               libDAio_testing
               libDAio_chain
