...
fastUnstakeSystem = './fast.refund/DAio.system/DAio.system.wasm'
...
def startNode(nodeIndex, account):
    ...
    if not nodeIndex: otherOpts += (
        '    --plugin DAio::history_plugin'
        '    --plugin DAio::history_api_plugin'
    )
    cmd = (
        args.nodeos +
        ...
        '    --plugin DAio::http_plugin'
        '    --plugin DAio::chain_api_plugin'
        '    --plugin DAio::producer_plugin' +
        otherOpts)
...
def createSystemAccounts():
    for a in systemAccounts:
        run(args.cleos + 'create account DAio ' + a + ' ' + args.public_key)
...
def createStakedAccounts(b, e):
    ...
    for i in range(b, e):
        ...
        retry(args.cleos + 'system newaccount --transfer DAio %s %s --stake-net "%s" --stake-cpu "%s" --buy-ram "%s"   ' % 
            (a['name'], a['pub'], intToCurrency(stakeNet), intToCurrency(stakeCpu), intToCurrency(ramFunds)))
        if unstaked:
            retry(args.cleos + 'transfer DAio %s "%s"' % (a['name'], intToCurrency(unstaked)))
...
def claimRewards():
    table = getJsonOutput(args.cleos + 'get table DAio DAio producers -l 100')
    times = []
...
def updateAuth(account, permission, parent, controller):
    run(args.cleos + 'push action DAio updateauth' + jsonArg({
        'account': account,
...
def msigProposeReplaceSystem(proposer, proposalName):
    ...
    trxPermissions = [{'actor': 'DAio', 'permission': 'active'}]
    with open(fastUnstakeSystem, mode='rb') as f:
        setcode = {'account': 'DAio', 'vmtype': 0, 'vmversion': 0, 'code': f.read().hex()}
    run(args.cleos + 'multisig propose ' + proposalName + jsonArg(requestedPermissions) + 
        jsonArg(trxPermissions) + 'DAio setcode' + jsonArg(setcode) + ' -p ' + proposer)
...
def msigReplaceSystem():
    run(args.cleos + 'push action DAio buyrambytes' + jsonArg(['DAio', accounts[0]['name'], 200000]) + '-p DAio')
    sleep(1)
...
def stepStartWallet():
    ...
    importKeys()
def stepStartBoot():
    startNode(0, {'name': 'DAio', 'pvt': args.private_key, 'pub': args.public_key})
    sleep(1.5)
def stepInstallSystemContracts():
    run(args.cleos + 'set contract DAio.token ' + args.contracts_dir + '/DAio.token/')
    run(args.cleos + 'set contract DAio.msig ' + args.contracts_dir + '/DAio.msig/')
def stepCreateTokens():
    run(args.cleos + 'push action DAio.token create \'["DAio", "10000000000.0000 %s"]\' -p DAio.token' % (args.symbol))
    totalAllocation = allocateFunds(0, len(accounts))
    run(args.cleos + 'push action DAio.token issue \'["DAio", "%s", "memo"]\' -p DAio' % intToCurrency(totalAllocation))
    sleep(1)
def stepSetSystemContract():
    retry(args.cleos + 'set contract DAio ' + args.contracts_dir + '/DAio.system/')
    sleep(1)
    run(args.cleos + 'push action DAio setpriv' + jsonArg(['DAio.msig', 1]) + '-p DAio@active')
def stepInitSystemContract():
    run(args.cleos + 'push action DAio init' + jsonArg(['0', '4,SYS']) + '-p DAio@active')
    sleep(1)
...
def stepLog():
    run('tail -n 60 ' + args.nodes_dir + '00-DAio/stderr')
...
commands = [
    ...
    ('s', 'sys',                createSystemAccounts,       True,    "Create system accounts (DAio.*)"),
    ...
    ('q', 'resign',             stepResign,                 True,    "Resign DAio"),
    ('m', 'msg-replace',        msigReplaceSystem,          False,   "Replace system contract using msig"),
    ('X', 'xfer',               stepTransfer,               False,   "Random transfer tokens (infinite loop)"),
    ('l', 'log',                stepLog,                    True,    "Show tail of node's log"),
]

parser.add_argument('--public-key', metavar='', help="DAio Public Key", default='DA8Znrtgwt8TfpmbVpTKvA2oB8Nqey625CLN8bCN3TEbgx86Dsvr', dest="public_key")
parser.add_argument('--private-Key', metavar='', help="DAio Private Key", default='5K463ynhZoCDDa4RDcr63cUwWLTnKqmdcoTKTHBjqoKfv4u5V7p', dest="private_key")
...
parser.add_argument('--symbol', metavar='', help="The DAio.system symbol", default='SYS')
