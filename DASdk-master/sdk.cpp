...
sdk::sdk(std::string const name, void *DAsdk_context)
   : acl_mgr_(0),
     agent_mgr_(0),
     aresolve_mgr_(0),
     class_map_mgr_(0),
...
// (to use in place of the brute-force one called "DASdk", see 
// /usr/lib/SysdbMountProfiles/DASdkAll), an app can be started with the env var 
// DA_PRINT_PROFILES_AND_EXIT pointing to a filename where the profile should be 
// written, then exit.
