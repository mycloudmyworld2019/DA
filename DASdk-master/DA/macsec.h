 ...
 * DA::macsec_profile_t profile("test1");
 * DA::macsec_key_t primary;
 * primary.ckn_is("aaaa");
 * primary.cak_is("123");
 * DA::macsec_key_t fallback;
 ...
 * DA::intf_id_t intfId("Ethernet1/1");
 ...
 * changes.
 */
class DA_SDK_PUBLIC macsec_handler :
 ...
 */
class DA_SDK_PUBLIC macsec_mgr : public base_mgr<macsec_handler, intf_id_t> {
 public:
   virtual ~macsec_mgr();
 ...
 protected:
   macsec_mgr() DA_SDK_PRIVATE;
   friend class macsec_handler;

 private:
   DA_SDK_DISALLOW_COPY_CTOR(macsec_mgr);
};

} // namespace DA 

#include <DA/inline/macsec.h>
