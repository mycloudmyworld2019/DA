...
class DA_SDK_PUBLIC intf_handler : public base_handler<intf_mgr, intf_handler> {
 public:
...
class DA_SDK_PUBLIC intf_iter_t : public iter_base<intf_id_t,
                                                    intf_iter_impl> {
 private:
   friend class intf_iter_impl;
   explicit intf_iter_t(intf_iter_impl * const) DA_SDK_PRIVATE;
};
 ...
 */
class DA_SDK_PUBLIC intf_mgr : public base_mgr<intf_handler, intf_id_t> {
 public:
   virtual ~intf_mgr();
   ...
   /**
    * Given a kernel interface name string, return the DA interface as an intf_id_t.
    * Returns an empty intf_id_t() if matching DA interface is not found.
    */
   virtual intf_id_t DA_intf_name(std::string) const = 0;
 ...
 protected:
   intf_mgr() DA_SDK_PRIVATE;
   friend class intf_handler;

 private:
   DA_SDK_DISALLOW_COPY_CTOR(intf_mgr);
};
 ...
 */
class DA_SDK_PUBLIC intf_counter_mgr {
 public:
 ...
 protected:
   intf_counter_mgr() DA_SDK_PRIVATE;
   
 private:
   DA_SDK_DISALLOW_COPY_CTOR(intf_counter_mgr);
};

}

#include <DA/inline/intf.h>
