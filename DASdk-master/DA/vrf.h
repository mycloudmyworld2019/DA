 ...
 *
 **/
class DA_SDK_PUBLIC vrf_handler : public base_handler<vrf_mgr, vrf_handler> {
 public:
...
class DA_SDK_PUBLIC vrf_iter_t : public iter_base<std::string, vrf_iter_impl> {
 private:
   friend class vrf_iter_impl;
   explicit vrf_iter_t(vrf_iter_impl * const) DA_SDK_PRIVATE;
};

/**
 * The manager for VRF, this is the main entry point for applications
 * to use DASdk VRF APIs.
 */
class DA_SDK_PUBLIC vrf_mgr : public base_mgr<vrf_handler, std::string> {
 public:
   virtual ~vrf_mgr();
 ...
 protected:
   vrf_mgr() DA_SDK_PRIVATE;
   friend class vrf_handler;
 private:
   DA_SDK_DISALLOW_COPY_CTOR(vrf_mgr);
};

} // end namespace DA

#include <DA/inline/vrf.h>
