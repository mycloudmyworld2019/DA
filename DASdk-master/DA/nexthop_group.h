...
/**
 * @file
 * Nexthop groups are a forwarding/tunneling abstraction in DA.
 *
 ...
 *
 * @code
 *   DA::nexthop_group_t nhg("nhg1", DA::NEXTHOP_GROUP_GRE);
 *   // Specify two destinations (nexthop group entries) for the traffic
 *   DA::nexthop_group_entry_t nhe1(DA::ip_addr_t("172.12.1.1"));
 *   DA::nexthop_group_entry_t nhe2(DA::ip_addr_t("172.12.1.2"));
...
#ifndef DA_NEXTHOP_GROUP_H
#define DA_NEXTHOP_GROUP_H
...
#include <DA/types/nexthop_group.h>
...
class DA_SDK_PUBLIC nexthop_group_handler :
...
class DA_SDK_PUBLIC nexthop_group_iter_t :
      public iter_base<nexthop_group_t, nexthop_group_iter_impl> {
 private:
   friend class nexthop_group_iter_impl;
   explicit nexthop_group_iter_t(nexthop_group_iter_impl * const) DA_SDK_PRIVATE;
};
 ...
 */
class DA_SDK_PUBLIC nexthop_group_mgr :
 ...
 protected:
   nexthop_group_mgr() DA_SDK_PRIVATE;
   friend class nexthop_group_handler;
 private:
   DA_SDK_DISALLOW_COPY_CTOR(nexthop_group_mgr);
};

} // end namespace DA

#include <DA/inline/nexthop_group.h>

#endif // DA_NEXTHOP_GROUP_H
