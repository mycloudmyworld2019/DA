...
#ifndef DA_IP_ROUTE_H
#define DA_IP_ROUTE_H
...
class DA_SDK_PUBLIC ip_route_iter_t : public iter_base<ip_route_t,
                                                        ip_route_iter_impl> {
 private:
   friend class ip_route_iter_impl;
   explicit ip_route_iter_t(ip_route_iter_impl * const) DA_SDK_PRIVATE;
};
...
class DA_SDK_PUBLIC ip_route_via_iter_t : public iter_base<ip_route_via_t,
                                                            ip_route_via_iter_impl> {
 private:
   friend class ip_route_via_iter_impl;
   explicit ip_route_via_iter_t(ip_route_via_iter_impl * const) DA_SDK_PRIVATE;
};


/// The manager for IP static route configuration
class DA_SDK_PUBLIC ip_route_mgr {
 public:
    ...
    * to this route, it can pass `DA::IP_ROUTE_ACTION_NEXTHOP_GROUP`,
    * which allows DA to more efficiently program routes. Routes are
    ...
    * DA will ECMP across these "forward" routes. If it programs a
    ...
    *
    * @throws DA::invalid_argument_error If the passed via has no IP
 ...
 protected:
   ip_route_mgr() DA_SDK_PRIVATE;
 private:
   DA_SDK_DISALLOW_COPY_CTOR(ip_route_mgr);
};
