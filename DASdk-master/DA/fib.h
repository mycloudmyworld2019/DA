...
#include <DA/types/fib.h>

namespace DA {

class fib_route_iter_impl;

class DA_SDK_PUBLIC fib_route_iter_t
    : public iter_base<fib_route_t, fib_route_iter_impl> {

 private:
   friend class fib_route_iter_impl;
   explicit fib_route_iter_t(fib_route_iter_impl * const) DA_SDK_PRIVATE;
};

class fib_fec_iter_impl;

class DA_SDK_PUBLIC fib_fec_iter_t
    : public iter_base<fib_fec_t, fib_fec_iter_impl> {

 private:
   friend class fib_fec_iter_impl;
   explicit fib_fec_iter_t(fib_fec_iter_impl * const) DA_SDK_PRIVATE;
};

class fib_mgr;

/// This class receives changes route/fec collection
class DA_SDK_PUBLIC fib_handler : public base_handler<fib_mgr, fib_handler> {
 public:
...
/// The FIB Manager
class DA_SDK_PUBLIC fib_mgr : public base_mgr<fib_handler> {
 public:
   virtual ~fib_mgr();
 ...
 protected:
   fib_mgr() DA_SDK_PRIVATE;
   friend class fib_handler;
 private:
   DA_SDK_DISALLOW_COPY_CTOR(fib_mgr);
};

#include <DA/inline/fib.h>
