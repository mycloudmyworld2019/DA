...
#ifndef DA_TYPES_IP_H
#define DA_TYPES_IP_H
 ...
 */
class DA_SDK_PUBLIC ip_prefix_t {
 public:
   ip_prefix_t();
   /**
 ...
 *
 * IPv6 addresses in DA only support contiguous masks, so for IPv6 addresses
 * mask() provides the same value as mask_length().
 */
class DA_SDK_PUBLIC ip_addr_mask_t {
 public:
   ip_addr_mask_t();
 ...
 */
bool parse_ip_addr(char const * addr, ip_addr_t * result) DA_SDK_PUBLIC;
 ...
 */
bool parse_ip_prefix(char const * addr, ip_prefix_t * result) DA_SDK_PUBLIC;


/** Tried to configure an internal VLAN on a trunk port. */
class DA_SDK_PUBLIC address_overlap_error : public configuration_error {
 public:
