...
#ifndef DA_MPLS_VRF_LABEL_H
#define DA_MPLS_VRF_LABEL_H
...
#include <DA/types/mpls_vrf_label.h>
 ...
 * DA::mpls_vrf_label_t vrf_label(mpls_label_t(16), "foo");
...
class DA_SDK_PUBLIC mpls_vrf_label_iter_t : public iter_base<
   mpls_vrf_label_t, mpls_vrf_label_iter_impl> {
 private:
   friend class mpls_vrf_label_iter_impl;
   explicit mpls_vrf_label_iter_t(mpls_vrf_label_iter_impl * const) DA_SDK_PRIVATE;
};
 ...
 * your DA::agent_handler::on_initialized virtual function is called, the manager
 * is valid for use.
 */
class DA_SDK_PUBLIC mpls_vrf_label_mgr {
 public:
 ...
 protected:
   mpls_vrf_label_mgr() DA_SDK_PRIVATE;
 private:
   DA_SDK_DISALLOW_COPY_CTOR(mpls_vrf_label_mgr);
};

}

#endif // DA_MPLS_VRF_LABEL_H
