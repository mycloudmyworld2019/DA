 ...
 *
 * In DA, an ACL is a collection of rules ordered by a sequence
 ...
 * used with a variety of DA features, including traffic filters on
 * interfaces (supported by this module) and as traffic classifiers
 * for class maps (DA/class_map.h).
 *
 ...
 * #include <DA/ip.h>
 *
 * // The result of DA::sdk::get_acl_mgr() is in variable
 * // acl_mgr_ within this example.
 *
 * // Deny all web traffic originating at srcaddr
 * DA::ip_addr_mask_t srcaddr(DA::ip_addr_t("192.0.2.1"), 32);
 * DA::ip_addr_mask_t dstaddr(DA::ip_addr_t("10.0.0.0"), 8);
 * DA::acl_key_t acl_key("name_of_acl", DA::ACL_TYPE_IPV4);
 * DA::acl_rule_ip_t rule1;
 *
 * rule1.source_addr_is(srcaddr);
 * rule1.destination_addr_is(dstaddr);
 * rule1.source_port_is(80);
 * rule1.action_is(DA::ACL_DENY);
 *
 ...
 * acl_mgr_->acl_apply(acl_key, DA::intf_id_t("Ethernet1"), DA::ACL_IN, true);
 * @endcode
 */
 ...
 */
class DA_SDK_PUBLIC acl_handler : public base_handler<acl_mgr, acl_handler> {
 public:
...
/// An ACL iterator.
class DA_SDK_PUBLIC acl_iter_t : public iter_base<acl_key_t, acl_iter_impl> {
 private:
   friend class acl_iter_impl;
   explicit acl_iter_t(acl_iter_impl * const) DA_SDK_PRIVATE;
};
...
class DA_SDK_PUBLIC acl_rule_ip_iter_t : public iter_base<acl_rule_ip_entry_t,
                                                           acl_rule_ip_iter_impl> {
 private:
   friend class acl_rule_ip_iter_impl;
   explicit acl_rule_ip_iter_t(acl_rule_ip_iter_impl * const) DA_SDK_PRIVATE;
};
...
class DA_SDK_PUBLIC acl_rule_eth_iter_t : public iter_base<acl_rule_eth_entry_t,
                                                            acl_rule_eth_iter_impl> {
 private:
   friend class acl_rule_eth_iter_impl;
   explicit acl_rule_eth_iter_t(acl_rule_eth_iter_impl * const) DA_SDK_PRIVATE;
};
 ...
 */
class DA_SDK_PUBLIC acl_mgr : public base_mgr<acl_handler> {
 public:
   virtual ~acl_mgr();
 ...
 protected:
   acl_mgr() DA_SDK_PRIVATE;
   friend class acl_handler;

 private:
   DA_SDK_DISALLOW_COPY_CTOR(acl_mgr);
};

}  // namespace DA

#include <DA/inline/acl.h>
