...
/**
 * @file
 * DA Agent handler and agent loop management.
 *
 * The agent_handler and agent_mgr provide a means to manage the agent
 * process life-cycle as managed by DA' ProcMgr/Launcher
 ...
 */
class DA_SDK_PUBLIC agent_handler : public base_handler<agent_mgr, agent_handler> {
 public:
...
class DA_SDK_PUBLIC agent_option_iter_t :
      public iter_base<std::string, agent_option_iter_impl> {
 private:
   friend class agent_option_iter_impl;
   explicit agent_option_iter_t(agent_option_iter_impl * const) DA_SDK_PRIVATE;
};

class agent_status_iter_impl;
/// Iterator type for published status keys.
class DA_SDK_PUBLIC agent_status_iter_t :
      public iter_base<std::string, agent_status_iter_impl> {
 private:
   friend class agent_status_iter_impl;
   explicit agent_status_iter_t(agent_status_iter_impl * const) DA_SDK_PRIVATE;
};


class DA_SDK_PUBLIC agent_mgr : public base_mgr<agent_handler> {
 public:
    virtual ~agent_mgr();
 ...
 protected:
    agent_mgr() DA_SDK_PRIVATE;
    friend class agent_handler;
 private:
    DA_SDK_DISALLOW_COPY_CTOR(agent_mgr);

};

}

#include <DA/inline/agent.h>
