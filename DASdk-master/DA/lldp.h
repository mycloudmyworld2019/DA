 ...
 * On DA, LLDP is disabled by default, but once enabled (cli: "lldp run"), all
 ...
 *
 * When enabled, DA transmits these TLVS on an interface (if they apply)
...
class DA_SDK_PUBLIC lldp_handler : public base_handler<lldp_mgr, lldp_handler> {
 public:
...
class DA_SDK_PUBLIC lldp_intf_iter_t : public iter_base<intf_id_t, 
                                                         lldp_intf_iter_impl> {
 private:
   friend class lldp_intf_iter_impl;
   explicit lldp_intf_iter_t(lldp_intf_iter_impl * const) DA_SDK_PRIVATE;
};

class lldp_remote_system_iter_impl;
/// An iterator that yields a lldp_remote_system_t for each neighbor on an interface
class DA_SDK_PUBLIC lldp_remote_system_iter_t : public 
 ...
 private:
   ...
   explicit lldp_remote_system_iter_t(
                               lldp_remote_system_iter_impl * const) DA_SDK_PRIVATE;
};
...
class DA_SDK_PUBLIC lldp_neighbor_iter_t : public iter_base<lldp_neighbor_t, 
                                                          lldp_neighbor_iter_impl> {
 private:
    friend class lldp_neighbor_iter_impl;
    explicit lldp_neighbor_iter_t(lldp_neighbor_iter_impl * const) DA_SDK_PRIVATE;
};
 ...
 */
class DA_SDK_PUBLIC lldp_mgr : public base_mgr<lldp_handler> {
 public:
   virtual ~lldp_mgr();
   ...
   /// In DA's case, the address family is an IPV4 address.
 ...
 protected:
   lldp_mgr() DA_SDK_PRIVATE;
   friend class lldp_handler;

 private:
   DA_SDK_DISALLOW_COPY_CTOR(lldp_mgr);
};
