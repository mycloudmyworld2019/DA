 ...
 * environments, instantiate an DA::sdk_scoped_lock before using any
 * DA::sdk features. Users do not need to grab a lock if they only
 ...
 *
 */
class DA_SDK_PUBLIC sdk_scoped_lock {
  public:
   sdk_scoped_lock();
   ~sdk_scoped_lock();
...
class DA_SDK_PUBLIC sdk {
 public:
    ...
    */
   explicit sdk(std::string const agent_name, void *DAsdk_context=NULL);
   virtual ~sdk();
    ...
    *
    * This variable is used to register the process with DA's various
    ...
    * agent. It first registers itself as an DA agent, then
 ...
 * signal handler.
 */
void default_signal_handler(int signal) DA_SDK_PUBLIC;
 ...
 * loop. It should only be rarely used, and future versions of DA
 * will eliminate the need for this method entirely. At that point,
 * this method will be removed, so please contact the DA SDK
 ...
 * the requested size.
 */
void internal_connection_buffer_size_is(uint32_t bytes) DA_SDK_PUBLIC;
