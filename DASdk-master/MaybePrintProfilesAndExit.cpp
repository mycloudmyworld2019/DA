...
// brute-force one "DASdk", see /usr/lib/SysdbMountProfiles/DASdkAll), an app can
// be started with the env var DA_PRINT_PROFILES_AND_EXIT pointing to a filename
...
// static class in order not to overly pollute the DA space. They are declared in
// DA/inline/sdk.h.
//
...
const char* print_profiles::DAsdk_progname = 0;
...
void print_profiles::set_print_profiles(const char* name) {
   if (getenv("DA_PRINT_PROFILES_AND_EXIT")) {
      print_profiles_fp = fopen(getenv("DA_PRINT_PROFILES_AND_EXIT"), "w");
      if (!print_profiles_fp) {
         const char* text =
                    "Error: DA_PRINT_PROFILES_AND_EXIT must be a writtable file\n";
...
void print_profiles::write_profiles() {
   if (print_profiles_fp) {
      ...
      for (std::map<std::string, bool>::iterator it = profiles_to_print.begin();
         ...
         fprintf(print_profiles_fp, "Include: DASdk_%s.include\n",
                                     it->first.c_str());
