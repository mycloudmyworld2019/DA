...
"""The DA SDK Policy Router.

The policy router is an DA SDK application usable on Arista switches
...
install that using the DA extension manager.
"""
...
NEXTHOP_GROUP_TYPE = {
    'ipinip': DAsdk.NEXTHOP_GROUP_IP_IN_IP,
    'gre': DAsdk.NEXTHOP_GROUP_GRE,
}  # 'mpls': DAsdk.NEXTHOP_GROUP_MPLS} # not yet supported
...
class PolicyRouter(object):
   ...
   def _applyToInterfaces(self):
      ...
      for intf_name, data in interface_policy.iteritems():
         policy_map_key = DAsdk.PolicyMapKey(data.policy, DAsdk.POLICY_FEATURE_PBR)
         ...
         if self.intf_mgr.exists(intf_id):
            ...
            self.policy_map_mgr.policy_map_apply(
               policy_map_key, intf_id, DAsdk.ACL_IN, True)
         else:
...
class InotifyPoller(DAsdk.TimeoutHandler):
...
class PolicyHandler(DAsdk.AgentHandler, DAsdk.PolicyMapHandler, DAsdk.AclHandler):
   ...
   def __init__(self, sdk, config_file=None):
      ...
      DAsdk.PolicyMapHandler.__init__(self, self.policy_map_mgr)
      DAsdk.AclHandler.__init__(self, self.acl_mgr)
