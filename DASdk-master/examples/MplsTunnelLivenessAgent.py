...
# Requires: RPMs for scapy and DASdk, as well as the DAsdk_utils.py
...
class MplsTunnelLivenessAgent(DAsdk_utils.DASdkAgent,
                              DAsdk.AgentHandler,
                              DAsdk.FdHandler,
                              DAsdk.TimeoutHandler):
   ...
   def __init__(self, sdk, config_file="MplsTunnelLivenessConfig.json"):
      """ Create the agent. Requires an DAsdk handle, as well as the
      input configuration """
      ...
      self.tracer = DAsdk.Tracer("MplsTunnelLivenessAgent")
      DAsdk_utils.DASdkAgent.__init__(self)
      DAsdk.AgentHandler.__init__(self, self.agent_mgr)
      DAsdk.TimeoutHandler.__init__(self, sdk.get_timeout_mgr())
   ...
   def on_initialized(self):
      ...
      src_ips = self.ip_intf_mgr.ip_addrs(DAsdk.IntfId(self.src_intf))
      if not src_ips:
   ...
   def on_timeout(self):
      ...
      self.timeout_time_is(DAsdk.now() + POLL_TIME)
   ...
   def resolve_egress_tunnel(self, tunnel):
      self.tracer.trace8("Resolve the nexthop IP %s to an ethernet address" %
                         tunnel.nexthop_ip)
      neighbor_key = DAsdk.NeighborKey(
         DAsdk.IpAddr(tunnel.nexthop_ip), DAsdk.IntfId())
      neighbor_entry = self.neighbor_table_mgr.neighbor_entry_status(neighbor_key)
      if neighbor_entry == DAsdk.NeighborEntry():
      ...
      if mac_entry.intf() == DAsdk.IntfId():
      ...
      if egress_eth_addr == DAsdk.EthAddr():
