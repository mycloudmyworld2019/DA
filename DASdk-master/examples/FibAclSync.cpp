...
class fib_acl_sync : public DA::agent_handler,
                     public DA::acl_handler,
                     public DA::fib_handler,
                     public DA::timeout_handler {
 public:
   DA::agent_mgr * agentMgr;
   explicit fib_acl_sync(DA::sdk & sdk)
      ...
      initialized = false;
      vrf = VRF_DEFAULT;
      route_types.insert(DA::ROUTE_TYPE_STATIC_CONFIG);
   ...
   void on_route_del(DA::fib_route_key_t const& route_key) {
         ...
         if (value.empty()) {
            ...
            while(rtype) {
               ...
               if (rtype == ROUTE_TYPE_BGP) {
                  route_types.insert(DA::ROUTE_TYPE_BGP);
               } else if (rtype == ROUTE_TYPE_IBGP) {
                  route_types.insert(DA::ROUTE_TYPE_IBGP);
               } else if (rtype == ROUTE_TYPE_EBGP) {
                  route_types.insert(DA::ROUTE_TYPE_EBGP);
               } else if (rtype == ROUTE_TYPE_BGP_AGGREGATE) {
                  route_types.insert(DA::ROUTE_TYPE_BGP_AGGREGATE);
   ...
   void schedule_acl_update(bool nodelay=false) {
      ...
      if (nodelay) {
         ...
         if (last_update_time + acl_update_timer > DA::now()) {
         ...
         timeout_time_is(DA::now() + acl_update_timer);
         t.trace3("%s: ACL update scheduled[ %f %f ]", __FUNCTION__,
                  DA::now(), acl_update_timer);
         update_status(STATUS_ACL_UPDATE_TIMER, acl_update_timer);
         pending_updates = true;
   ...
   void flush_all_rules_in_acls() {
      ...
      if (v4_acl_configured && (acl4_seq >= ACL_SEQ_INCR)) {
         ...
         auto acl4 = DA::acl_key_t(ipv4_access_list_name, DA::ACL_TYPE_IPV4);
         while (seq < acl4_seq) {
      ...
      if (v6_acl_configured && (acl6_seq >= ACL_SEQ_INCR)) {
         ...
         auto acl6 = DA::acl_key_t(ipv6_access_list_name, DA::ACL_TYPE_IPV6);
         while (seq < acl6_seq) {
   ...
   std::string convert(std::set<DA::fib_route_type_t> & type) {
      bool start = true;
      std::string route_types_str;
      for (std::set<DA::fib_route_type_t>::iterator itr=type.begin();
           itr!=type.end(); ++itr) {
 ...
 private:
   ...
   std::set<DA::fib_route_type_t>route_types;
   uint32_t acl4_seq;
   uint32_t acl6_seq;
   bool initialized;
};
