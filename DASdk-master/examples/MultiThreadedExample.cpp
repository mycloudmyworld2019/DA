...
void *run_child(void *mgr) {
   ...
   for(int i = 0; i < 1000; i++) {
      ...
      ((DA::agent_mgr *) mgr)->status_set("index", status);
...
class parent_handler_agent : public DA::agent_handler {
 public:
   explicit parent_handler_agent(DA::agent_mgr * mgr)
         : DA::agent_handler(mgr) {
...
int main(int argc, char ** argv) {
   DA::sdk sdk;
   DA::agent_mgr * mgr = sdk.get_agent_mgr();
