...
class nexthop_counter_agent : public DA::agent_handler,
                              public DA::timeout_handler {
 public:
   explicit nexthop_counter_agent(DA::sdk & sdk)
   ...
   void on_initialized() {
      ...
      timeout_time_is(DA::now() + nexthop_counter_interval);
   }
   ...
   void display_counters() {
      ...
      for (auto nhIter = nhgMgr->nexthop_group_iter(); nhIter; ++nhIter) {
         ...
         for (uint16_t i = 0; i < read_nhg.size(); ++i) {
            ...
            if (!entries[ i ].nexthop() == DA::ip_addr_t()) {
