...
//    bash# g++ -lDA -std=gnu++14 examples/MplsOverGreNexthopGroups.cpp
...
//    bash# TRACE=DASdk* /mnt/flash/MplsOverGreNexthopGroups
// or from the CLI
...
class mpls_over_gre_nhg_agent : public DA::agent_handler {
 public:
   ...
   explicit mpls_over_gre_nhg_agent(DA::sdk & sdk)
   ...
   void create_nexthop_group(const std::string & name,
                             const DA::ip_addr_t & src_ip,
                             DA::intf_id_t src_intf) {
      t.trace0("Creating nexthop group");
      DA::nexthop_group_t nhg = 
         DA::nexthop_group_t(name,
                              DA::NEXTHOP_GROUP_MPLS_OVER_GRE );
      if ( !!src_intf ) {
   ...
   void set_nexthop_group_entry(const std::string & name,
                                ...
                                const DA::ip_addr_t & dest_ip) {
      t.trace0("Set nexthop group entry");
      assert(nhgMgr_->exists(name));
      DA::nexthop_group_t nhg = nhgMgr_->nexthop_group(name); 
      DA::nexthop_group_mpls_action_t mpls_action(DA::MPLS_ACTION_PUSH);
      mpls_action.label_stack_set(label);
      DA::nexthop_group_entry_t nhe(dest_ip);
   ...
   void on_initialized() {
      ...
      create_nexthop_group("MplsOverGreNhg1",
                           DA::ip_addr_t("10.0.0.1"),
                           DA::intf_id_t("Ethernet3") );
      set_nexthop_group_entry("MplsOverGreNhg1",
                              0, DA::mpls_label_t(30),
                              DA::ip_addr_t("10.0.0.33"));
      set_nexthop_group_entry("MplsOverGreNhg1",
                              1, DA::mpls_label_t(40),
                              DA::ip_addr_t("10.0.0.44"));
      // Standby Nexthop group
      create_nexthop_group("MplsOverGreNhg2",
                           DA::ip_addr_t("20.0.0.1"),
                           DA::intf_id_t("Ethernet4") );
      set_nexthop_group_entry("MplsOverGreNhg2",
                              0, DA::mpls_label_t(30),
                              DA::ip_addr_t("20.0.0.33"));
      set_nexthop_group_entry("MplsOverGreNhg2",
                              1, DA::mpls_label_t(40),
                              DA::ip_addr_t("20.0.0.44"));

      routeKey = DA::ip_route_key_t(DA::ip_prefix_t(
                                     DA::ip_addr_t("172.1.1.4"), 32));
