...
class DirectFlowReprogrammer : public DA::flow_handler {
 public:
   std::unordered_map<std::string, DA::flow_entry_t> requests;
   DA::directflow_mgr * mgr_;
   DA::tracer tracer;
   explicit DirectFlowReprogrammer(DA::sdk & sdk)
         : DA::flow_handler(sdk.get_directflow_mgr()),
           requests(),
           mgr_(sdk.get_directflow_mgr()),
           tracer("DASdkDirectFlowReprogrammer") {
      watch_all_flows(true);
      tracer.trace0("Constructed");
   }

   void updateFlow(DA::flow_entry_t const & entry) {
      if(mgr_->exists(entry.name())) {
         ...
         DA::flow_entry_t entryCopy(entry.name() + TMP_ENTRY_SUFFIX,
                                     entry.match(),
                                     entry.action(),
                                     entry.priority() + 1);
   ...
   void on_flow_status(std::string const & name, DA::flow_status_t status) {
      ...
      } else if(status == DA::FLOW_DELETED) {
         if(!tmpEntry) {
      ...
      } else if(status == DA::FLOW_REJECTED) {
