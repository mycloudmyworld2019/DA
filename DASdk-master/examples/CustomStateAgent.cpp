...
class custom_state_agent : public DA::agent_handler {
 public:
   DA::agent_mgr * agent_mgr;
   DA::tracer t;
   
   explicit custom_state_agent(DA::sdk & sdk)
   ...
   void connect_to_controller(std::string const & ip_str) {
      ...
      bool valid = DA::parse_ip_addr(ip_str.c_str(), &address);
      if(!valid) {
