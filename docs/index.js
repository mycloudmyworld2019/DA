let DA_SALE        = "0xd0a6e6c54dbc68db5db3a091b171a77407ff7ccf"
let DA_SALE_UTIL   = "0x860fd485f533b0348e413e65151f7ee993f93c02"
let DA_TOKEN       = "0x86fa049857e0209aa7d9e616f7eb3b3b78ecfdb0"

let DA_sale, DA_token
...
let getDailyTotals = async () => words32(await call({
  to: DA_SALE_UTIL, data: sighash("dailyTotals()"),
})).map(web3.toBigNumber)

let getUserBuys = async address => words32(await call({
  to: DA_SALE_UTIL, data: calldata(sighash("userBuys()"), address),
})).map(web3.toBigNumber)

let getUserClaims = async address => words32(await call({
  to: DA_SALE_UTIL, data: calldata(sighash("userClaims()"), address),
})).map(Number).map(Boolean)

let getKey = async address => bytes(
  words32(await call({
    to: DA_SALE, data: calldata(sighash("keys(address)"))
  })).slice(2).map(unhex).join("").replace(/(00)*$/, "")
).map(Number).map(String.fromCharCode).join("")

addEventListener("mousemove", event => DA_ecc.key_utils.addEntropy(
  event.pageX, event.pageY, event.screenX, event.screenY
), { capture: false, passive: true })

onload = () => {
  if (window.web3) {
    DA_sale  = web3.eth.contract(DA_SALE_ABI).at(DA_SALE)
    DA_token = web3.eth.contract(DA_TOKEN_ABI).at(DA_TOKEN)
...
let refresh = () => new Promise(resolve => async.parallel(Object.assign({
  ...
  DA_balance: toAsync(balanceOf(DA_TOKEN, web3.eth.accounts[0])),
  ...
  dailyTotals, eth_balance, DA_balance, publicKey, userBuys, userClaims,
}) => {
  let time = getTime()
  ...
  resolve(update({
    time, days, unclaimed, eth_balance, DA_balance, publicKey
  }))
})))

let render = ({
  time, days, unclaimed, eth_balance, DA_balance, publicKey, buyWindow,
}) => <div>
  <p className="hidden" style={{ width: "95%" }}>

    The DA Token Distribution will take place over about 341 days.
    1,000,000,000 (one billion) DA tokens will be created at the
    start of the sale, 100,000,000 DA are allocated to block.one and cannot be
    transfered.

    The remaining 900,000,000 DA will be split into different rolling windows of
    availability.  The DA tokens in a given window will be split
    proportionally to all ETH contributions made during that window.
    200,000,000 DA will be sold in the first window, lasting five days.
    The remaining 700,000,000 will be divided equally into 350 windows, each
    lasting 23 hours and distributing 2,000,000 DA.  Contributions can be made to
    any future window, but the DA cannot be claimed until the window closes.

    Once a window closes, the DA tokens allocated to that window are
    available to be claimed.

    You must generate and register an DA Public Key or it will not be possible for
    anyone to include your DA tokens in the genesis block of any future blockchain's
    based on DA.IO software.
  ...
  href="https://github.com/DAio/DA-token-distribution">contract source
  code</a>.
  </p>
  ...
  {web3.eth.accounts[0] ? <div>
    <div className="account pane">
      <h2 style={{ textAlign: "center" }}>DA Token Distribution</h2>
      <div className="info"><table><tbody>
        ...
        <tr>
          <th></th>
          <td style={{ textAlign: "left" }}>
            {formatDA(unclaimed)} DA (unclaimed)
          </td>
        </tr>
        <tr>
          <th></th>
          <td style={{ textAlign: "left" }}>
            {formatDA(DA_balance.div(WAD))} DA
          </td>
        </tr>
      </tbody></table></div>
        ...
        <div className="row" style={{ marginBottom: "1rem" }}>
          ...
          <div className="button" style={{ marginLeft: "1rem" }}>
            <a href="#" id="generate-link"
              ...
              { publicKey ? "Change your DA key" : "Generate DA key"}
            </a>
          </div>
        </div>
        <div className="row">
    ...
    <form className="hidden pane" id="generate-pane"
      ...
      <div id="generate-confirm" className="hidden">
        <h3>{publicKey ? "Change" : "Register"} DA key</h3>

        {publicKey ? <p>This will replace your DA claim key:
          <table>
            <tbody>
              <tr>
                <th>Public key</th>
        ...
        <table>
          <tbody>
            <tr>
              <th>Description</th>
              <td style={{ textAlign: "left" }}>
                DA Token Distribution Claim Key
              </td>
            </tr>
            <tr>
              <th>Public key</th>
        ...
        <p>
          ...
          it right now or you will be unable to access your DA tokens
          when the sale ends.
    ...
    <div className="hidden pane" id="register-pane">
      <h3>{publicKey ? "Change" : "Register"} DA public key</h3>
      <table>
        <tbody>
          <tr>
            <th>Public key</th>
    ...
    <form className="hidden pane" id="buy-pane"
      ...
      <table><tbody>
        ...
        <tr>
          <th>DA Distributed</th>
          <td style={{ textAlign: "left" }}>
            {formatDA(days[buyWindow].createOnDay)} DA
          </td>
        </tr>
        <tr>
          <th>Total ETH</th>
        ...
        <tr>
          ...
          <td style={{ textAlign: "left" }}>
            {days[buyWindow].price.toFormat(9)} ETH/DA
          </td>
        </tr>
        <tr>
          <th>Send ETH</th>
    ...
    <form className="hidden pane before-error" id="transfer-pane"
      ...
      <h3>Transfer DA tokens to another Ethereum account</h3>
      <table><tbody>
        <tr>
        ...
        <tr>
          ...
          <td style={{ textAlign: "left" }}>
            <input placeholder={formatDA(DA_balance.div(WAD))}
        ...
        <tbody>
          {days.map((day, i) =>
            <tr key={i} className={i == dayFor(time) ? "active" : i < dayFor(time) ? "closed" : ""}>
              ...
              <td>{formatDA(day.createOnDay)} DA</td>
              <td>{formatETH(day.dailyTotal)} ETH</td>
              <td>{day.dailyTotal == 0 ? "n/a" : (
                `${day.price.toFormat(9)} ETH/DA`
              )}</td>
              <td>{day.ends.fromNow()}</td>
              <td>{formatETH(day.userBuys)} ETH</td>
              <td>
                {formatDA(day.received)} DA
                {i >= dayFor(time)
                  && <span title="Pending DA subject to change if additional funds received" style={{ cursor: "pointer" }}> *</span>}
              </td>
            </tr>
          )}
        </tbody>
...
function buy() {
  ...
  DA_sale.buyWithLimit(state.buyWindow, 0, {
...
function transfer() {
  ...
  DA_token.transfer(guy, wad, hopefully(result => ping(result).then(() => {
    hidePanes()
