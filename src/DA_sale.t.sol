...
contract TestUser is DSExec {
    ...
    function TestUser(TestableDASale sale_) {
        sale = sale_;
    }
...
contract TestOwner {
    ...
    function TestOwner(TestableDASale sale_) {
        sale = sale_;
    }
...
contract TestableDASale is DASale {
    function TestableDASale(
...
contract DASaleTest is DSTest, DSExec {
    TestableDASale  sale;
    DSToken          DA;
    DSGuard          guard;
    TestUser         user1;
    ...
    function testBuyFirstDay() {
        ...
        sale.addTime(1 days);
        sale.claim(0);
        assertEq(DA.balanceOf(this), 31.25 ether);
    }
    ...
    function testBuyFirstAndSecondDay() {
        ...
        assertEq(DA.balanceOf(this), 54.25 ether);
    }
    ...
    function testAllDistributed() {
        ...
        assertEq(DA.balanceOf(this), 146.25 ether);
    }
    ...
    function testMultiUser() {
        ...
        assertEq(DA.balanceOf(this), 15.625 ether);
        assertEq(DA.balanceOf(user1), 15.625 ether);
    }

    function testMultiUserAsymmetricBid() {
        nextRound(1 ether, 9 ether, 0);
        assertEq(DA.balanceOf(this), 3.125 ether);
        assertEq(DA.balanceOf(user1), 28.125 ether);
    }

    // is this an issue?
    function testRepeatingDecimalRoundUp() {
        nextRound(1 ether, 1 ether, 1 ether);
        assertEq(DA.balanceOf(this), 10416666666666666667);
        assertEq(DA.balanceOf(user1), 10416666666666666667);
        assertEq(DA.balanceOf(user2), 10416666666666666667);
    }
    ...
    function testRepeatingDecimalRoundDown() {
        ...
        assertEq(DA.balanceOf(this), 19166666666666666665);
        assertEq(DA.balanceOf(user1), 3833333333333333333);
    }
    ...
    function testMultiUserFinalize() {
        ...
        nextRound(1 ether, 1 ether, 0);
        nextRound(1 ether, 1 ether, 0);
        assertEq(DA.balanceOf(this), 73.125 ether);
        assertEq(DA.balanceOf(user1), 73.125 ether);
        addTime();
    ...
    function testMultiUserAsymmetricBidFinalize() {
        ...
        assertEq(DA.balanceOf(this), 70.675 ether);
        assertEq(DA.balanceOf(user1), 41.075 ether);
        assertEq(DA.balanceOf(user2), 34.5 ether);
        addTime();
    ...
    function testClaimAll() {
        ...
        sale.claimAll();
        assertEq(DA.balanceOf(this), 77.25 ether);
    }
...
contract DASalePreInitTests is DSTest {
    TestableDASale  sale;
    TestUser         user1;
    ...
    function testFailTokenAuthority() {
        DSToken DA = new DSToken("DA");
        DA.setAuthority(new DSGuard());
        DA.setOwner(sale);
        sale.initialize(DA);
    }
}
