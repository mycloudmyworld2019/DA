...
import "DA-auth/auth.sol";
import "DA-exec/exec.sol";
import "DA-math/math.sol";

import "DA-token/token.sol";

contract DASale is DAAuth, DAExec, DAMath {
    DAToken  public  DA;                  // The DA token itself
    uint128  public  totalSupply;          // Total DA amount created
    ...
    function initialize(DAToken DA) auth {
        assert(address(DA) == address(0));
        assert(DA.owner() == address(this));
        assert(DA.authority() == DAAuthority(0));
        assert(DA.totalSupply() == 0);

        DA = DA;
        DA.mint(totalSupply);

        // Address 0xb1 is provably non-transferrable
        DA.push(0xb1, foundersAllocation);
    ...
    // encoded using the STEEM, BTS, or DA public key format.
    ...
    // CrowDAale owners can collect ETH any number of times
    ...
    // Anyone can freeze the token 1 day after the sale enDA
    function freeze() {
        assert(today() > numberOfDays + 1);
        DA.stop();
        LogFreeze();
